...
#define SET_TEMP_MAX_STEP_MIN           'G'   //установка праметров на странице PID_SETUP
#define GET_TEMP_MAX_STEP_MIN           'H'   //получение параметров от страницы PID_setting
...
unsigned int Input;          //текущая температура
int Output = MAXIMUM_HEATING ;           // коэфф. заполн. в относительных единицах 
...
time_t t_start_heat = 0;      //время начала нагрева на очередном проходе
time_t t_new_setpoint = 0;    //время начала нагрева с новым значением setpoint (начало следующего интервала)
#define SAVE_FILE 30 //частота сохранения in sec
#define TIME_UNSTABLE_SYSTEM 300 // sec, время , при котором считаем , что система неустойчива
//номер шага(участка температурного интервала )  
uint8_t num_step = 1;    //max=(temp_end-temp_start)/temp_step;

//номер ( прохода по интервалу температур) измерения времени регулирования при заданных коэффициентах
uint16_t num_run = 1;  
#define INF 1000000000 //время регулирования при нестабильной системе
uint16_t temp_step = 50 ;//шаг изменения температуры 
uint16_t temp_start = 50;  //начальная температура, от которой идёт настройка
uint16_t temp_end   = 1300;//конечная температура, до которой идёт настройка
uint8_t  num_step_max = (temp_end-temp_start)/temp_step; //количество шагов прохода по интервалу

//экстремальные значения производной
int max_derivative =  10 ;    //максимальное значение производной от температуры     
int min_derivative = -30 ;    //минимальное значение производной от температуры  
...
struct 
{
    ...
    unsigned flag_true_init                                     : 4;   //флаг успешной инициализации (2 - успешно, любое другое знач. — неуспешно)
...
File file_obj, file_obj1,file_obj2,file_obj3,file_obj4; // вспомогательный объект файла 
#endif