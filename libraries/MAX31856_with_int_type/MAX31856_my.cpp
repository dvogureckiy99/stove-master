...
unsigned int	MAX31856::readThermocouple(byte unit)
{
    unsigned int temperature;
    long data; //4 байта / 32 бита
    ...
    // Если нет связи с IC, тогда данные будут все 1, потому что внутреннего pullup на линии данных (INPUT_PULLUP)
    if (data == 0xFFFFFFFF)
        return NO_MAX31856;
    ...
    if (data & SR_FAULT_OPEN)
        ...
        //Отрицательные температуры были автоматически обработаны сдвигом выше
        // Convert to Celsius
        temperature = (unsigned int) data * 0.0078125;
...
unsigned int	MAX31856::readJunction(byte unit)
{
    unsigned int temperature;
...
void MAX31856::printTemperature(double temperature)
{
